...
- **Time window**: Last 7 days. **CRITICAL: Calculate the Unix timestamp correctly based on the current date provided in the system context (e.g., "Today's date is YYYY-MM-DD").** Do NOT hardcode or guess timestamps. The formula: convert today's date to Unix epoch, then subtract 604800 (7 days in seconds). For reference, 2026-01-01 00:00:00 UTC = 1767225600. Add 86400 per day from there.
- Pull up to 100 messages to capture all weekly updates
- **Validation**: If the returned messages are all older than 7 days, the timestamp was calculated wrong. Recalculate and retry.

### Step 2 - Filter to weekly update posts ONLY

- From the messages pulled in Step 1, filter to **only** posts that are personal weekly updates from AIC members
- **Detection patterns** (case-insensitive): messages where the header contains "update" (e.g., "Ben update :thread:", "OMAI Update"), OR where the header is just a person's name/initials followed by `:thread:` (e.g., "jh :thread:"). The pattern is: `[Name/initials] [optional "update"] :thread:` or `[Team] update`
- **Ignore** all other threads - discussion threads, questions, agenda posts, competitive intel shares, presentation suggestions, scheduling messages, join notifications, etc. Only the personal weekly update posts matter
...
| Karén Simonyan | U06TWRQM28N | `<@U06TWRQM28N>` |
| Prashant Dhamdhere | U09UWSUGZEJ | `<@U09UWSUGZEJ>` |
| Yunchao Gong | U08NL5V4VM5 | `<@U08NL5V4VM5>` |
| Purvi Shah | U09A3TCBSEB | `<@U09A3TCBSEB>` |
| Jeff Nettleton | U06U9ME81V3 | `<@U06U9ME81V3>` |
| Dave Citron | U099WR35UUW | `<@U099WR35UUW>` |
| Ben Sorscher | U06UAGMMT1B | `<@U06UAGMMT1B>` |
| David So | U08P4FWUW1J | `<@U08P4FWUW1J>` |
| Mark Wagner | U06URTVTW0H | `<@U06URTVTW0H>` |
| Steve Gaffney | U090AHG3PMW | `<@U090AHG3PMW>` |
| Weizhu Chen | U0703UWVBPH | `<@U0703UWVBPH>` |
| Xiaoliang Dai | U09BFD4BT2M | `<@U09BFD4BT2M>` |
| jhoffmann | U06TYMMGGQ7 | `<@U06TYMMGGQ7>` |
| Nando de Freitas | U07NAGAGNRE | `<@U07NAGAGNRE>` |
| Adam Sadovsky | U090VE3KHB4 | `<@U090VE3KHB4>` |
